...
<head>
    ...
    <script>
        ...
        {*/
            ...
            var keywords1="<?php echo $_GET['keywords1'];?>";
                    ...
                    .success(function(result) {
                        ...
                        if(obj != '')
                        {
                            $.each(obj, function(key, val) {
                                ...
                                load(branchData,branchCode,branchName,branchType,branchTel,branchAddress,branchContact,email,start,end,keywords,keywords1,keywords2,select_top,b);
                            });
                        }
                    });
            });
			}else if(keywords1!='cheese'){
				 $.get( "../path.txt", function( data ) {
                var resourceContent = data;
                $.ajax({
                    url: resourceContent+"/getDataBranch_.php?branchID="+keywords1,
                    type: "POST",
                    data: ''
                })
                    .success(function(result) {
        ...
        //}
        function load(branchData,branchCode,branchName,branchType,branchTel,branchAddress,branchContact,email,start,end,keywords,keywords1,keywords2,select_top,session) 		{
                    ...
                    .success(function(result) {
                        ...
                        if(obj != '')
                        {
							$("#load").text("กำลังโหลดไฟล์...");
								$.post('Print-Summary-All-Product-F.php', { 
								resultData:result,
								date_start:start,
								branch:branchName,
								date_end:end
   								...
   								//alert(result); 			
								window.location = 'MyPDF/File-Summary-All-Product-F.pdf';
							});
                            /*$.each(obj, function (key, val) {
