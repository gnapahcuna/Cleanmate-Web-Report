...
function BasicTable($header,$data,$getDate_start,$getDate_end,$getBranch)
{
	...
	$this->Cell(12,7,iconv( 'UTF-8','TIS-620','ลำดับ'),1, 'C', 'C',true);
	$this->Cell(22,7,iconv( 'UTF-8','TIS-620','ประเภทร้าน'),'1','C','C',true);
	$this->Cell(14,7,iconv( 'UTF-8','TIS-620','รหัสร้าน'),1,'C','C',true);
	$this->Cell(38,7,iconv( 'UTF-8','TIS-620','ชื่อร้าน'),1,'C','C',true);
    $this->Cell(22,7,iconv( 'UTF-8','TIS-620','จำนวนออเดอร์'),1,'C','C',true);
    $this->Cell(22,7,iconv( 'UTF-8','TIS-620','จำนวนเงิน'),1,'C','C',true);
	$this->Cell(24,7,iconv( 'UTF-8','TIS-620','ส่วนลดโดยร้าน'),1,'C','C',true);
   	...
   	foreach ($data as $eachResult) 
	{
		...
		$this->SetFont('AngsanaNew','',13);
		$this->Cell(12,6,number_format($j+1),1,'C','C');
		$this->Cell(22,6,iconv( 'UTF-8','TIS-620',$eachResult["BranchType"]),1,'C','C');
        $this->Cell(14,6,iconv( 'UTF-8','TIS-620',$eachResult["BranchCode"]),1,'C','C');
        $this->Cell(38,6,iconv( 'UTF-8','TIS-620',$eachResult["BranchNameTH"]),1,'C','C');
		
		if($eachResult["counts"]!=0){
			$this->Cell(22,6,number_format($eachResult["counts"]),1,'C','C');
		}else{
			$this->Cell(22,6,'-',1,'C','C');
		}if($eachResult["total"]!=0){
			$this->Cell(22,6,number_format($eachResult["total"],2),1,'C','C');
		}else{
			$this->Cell(22,6,'-',1,'C','C');
		}if($eachResult["SpecialDiscount"]!=0){
			$this->Cell(24,6,number_format($eachResult["SpecialDiscount"],2),1,'C','C');
		}else{
			$this->Cell(24,6,'-',1,'C','C');
		...
		if($j==sizeof($data)){
			...
			$this->Cell(86,7,iconv( 'UTF-8','TIS-620','รวม'),1,'C','C',true);
			if($data1!=0){
				$this->Cell(22,7,iconv( 'UTF-8','TIS-620',number_format($data1)),1,'C','C',true);
			}else{
				$this->Cell(22,7,iconv( 'UTF-8','TIS-620','-'),1,'C','C',true);
			}
			if($data2!=0){
				$this->Cell(22,7,iconv( 'UTF-8','TIS-620',number_format($data2,2)),1,'C','C',true);
			}else{
				$this->Cell(22,7,iconv( 'UTF-8','TIS-620','-'),1,'C','C',true);
			}
			if($data3!=0){
				$this->Cell(24,7,iconv( 'UTF-8','TIS-620',number_format($data3,2)),1,'C','C',true);
			}else{
				$this->Cell(24,7,iconv( 'UTF-8','TIS-620','-'),1,'C','C',true);
            }
            if($data4!=0){
				$this->Cell(36,7,iconv( 'UTF-8','TIS-620',number_format($data4,2)),1,'C','C',true);
			}else{
...
function Footer()
{
    ...
    $this->Cell(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
		...
		$pdf->Output("MyPDF/File-Order-All-F.pdf","F");
