	...
	$date_end=$_GET['date_end'];
				...
				$arrBranchCode=array();
								...
								while($row = sqlsrv_fetch_array($query)){
										array_push($arrBranchCode,$row['BranchCode']);
								...
								while($row1 = sqlsrv_fetch_array($query1)){
										...
										array_push($arrBranchID,$row1['BranchID']);
								}
								for($i=0;$i<count($arrPromo);$i++){
									array_push($object_array,array('BranchTypeNameTH' => $arrBranchType[$i], 'BranchCode' => $arrBranchCode[$i]
		,'BranchNameTH' => $arrBranchName[$i],'BranchID' => $arrBranchID[$i]));
								}
